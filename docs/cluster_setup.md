...
Notebook `05` begrenzt den Kafka-Nachweis mit `KAFKA_CONNECTION_TIMEOUT_SECONDS` und `KAFKA_OPERATION_TIMEOUT_MS`. Die Ausgaben `kafka_phase=tcp_preflight`, `producer_metadata`, `consumer_offsets`, `producer_send` und `consumer_poll` zeigen, in welcher Netzwerkphase eine FH-Konfiguration scheitert.

Wenn eine ältere Notebook-Ausführung bereits in der Kafka-Zelle hängt, brechen Sie den Kernel-Lauf einmal ab und starten den Kernel nach dem Aktualisieren von Notebook `05` neu. Bleibt der Lauf bei `tcp_preflight`, ist der konfigurierte Broker-Port aus dem JupyterHub nicht erreichbar. Scheitert er nach `producer_metadata`, muss zusätzlich der vom Broker beworbene advertised listener geprüft werden.
